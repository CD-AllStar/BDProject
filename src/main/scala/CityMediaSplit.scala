...
object CityMediaSplit {

  def main(args: Array[String]): Unit = {

    val conf = new SparkConf()
    conf.setAppName("Media-Split")
...
//    conf.set("spark.executor.memory","8g")
    ...
    val city_name = city_names.rdd.map(x=>((x.getString(0),x.getString(1),x.getString(2),x.getString(3),x.getString(5)),x.getString(4)))
      ...
      .filter(x=>x._2.split("\t")(1).startsWith("mm"))
...
//    city_name.take(10).foreach(println)
    ...
    city_name.toDF.registerTempTable("CityMediaSplit")
//
    hiveContext.sql("insert overwrite table city_media_split select uid,country,province,city,word,source,source_id,synonym from CityMediaSplit")
    sc.stop()
  }
}
